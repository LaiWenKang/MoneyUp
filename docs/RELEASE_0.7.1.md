...
| Swipe between tabs | An optional Settings gesture moves one tab left or right after a deliberate horizontal swipe on passive space, never wraps at the ends, and never removes the visible tab bar. Category/filter bars and other horizontal controls retain their own swipes. |
...
| Five-tab organization | The five permanent tabs remain Today, History, Log, Plan, and Assets. Plan opens directly on Budget and switches among Budget, Calendar, Goals, and Allowances with one larger, readable control; History hot-category controls use matching professional typography and tap targets. |
...
- release-asset validation, including 1,377 bilingual strings across three catalogs;
...
- 118 Python validator tests across the architecture, platform-action, and
  performance suites;
- 779 declared Swift tests: 726 XCTest and 53 Swift Testing declarations;
