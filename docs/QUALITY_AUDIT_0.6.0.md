...
| A-019 | P1 | TestFlight validated one exported IPA but used a second `exportArchive` operation for upload, so the transferred bytes were not proven identical. The workflow now pins and cross-job compares the Xcode build, SDK, and macOS runner family; records the exact signing runner image in the recovery fingerprint; exports once; records the IPA SHA-256; validates and `altool`-uploads that same path; and checks recovery-tree entry type/path/mode/symlink target/file size/content for the xcarchive (including dSYMs) and export directory plus exact IPA bytes after decryption. Owner, timestamps, xattrs, and ACLs are intentionally outside that digest. The validator rejects weaker toolchain checks, missing provenance/tree proof, or a second export/upload binary. | `.github/workflows/testflight.yml`; `Scripts/validate_release_assets.py`; local symlink/mode/dSYM round-trip fixture passed; authenticated exact-candidate run open |
...
| A-050 | P1 release availability | Independently scheduled TestFlight jobs compared GitHub's rolling `ImageVersion`, so run `33653864051` rejected two runners that had the same reviewed Xcode 26.6 build 17F113, iPhoneOS SDK 26.5, and macOS 26 family. Cross-job equality now uses only those stable reviewed identities; each exact runner image remains logged, and the signing image remains bound into the encrypted recovery fingerprint. The validator rejects reintroducing volatile `ImageVersion` into cross-job equality. | `.github/workflows/testflight.yml`; `Scripts/validate_release_assets.py`; TestFlight run `33653864051`; exact-head CI required |
...
Xcode build, SDK, and macOS runner family; it records each exact runner image
and binds the signing image into recovery evidence. It also fails unless the archive/dSYM/export tree and
